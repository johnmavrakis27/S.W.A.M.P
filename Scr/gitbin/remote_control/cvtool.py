...
class CVTool:
    ...
    def calibrate_color(self, folder_path, color_name="black", new_photos=True):
        '''
        ...
        user_input = input("Do you want to calibrate now (y/n):")
        ...
        for i, image in enumerate(images):
            ...
            while True:                
                ...
                img_contours, contours = self.find_contours(image, mask, min_area=1000, sort=True, filter=0, draw=True)
    ...
    def cnf_validation(self):
        data_names = ['red1', 'red2', 'green', 'blue_line', 'orange_line', 'wall']
        data = {}
            ...
            for color_name2 in data:
                ...
                for i in range(3):
                    ...
                    # if lower1 == lower2 we have conflict!
                        ...
                        conflicts += 1
                if conflicts == 3:
                    print("Three conflicts in colors", color_name1, "and", color_name2, "!!!")
                    return False
    ...
    def initTrackbars(self, hsv_lower, hsv_upper):
        ...
        cv2.createTrackbar("Hue Min","TrackBars", hsv_lower[0], 179, self.empty)
        cv2.createTrackbar("Hue Max","TrackBars", hsv_upper[0], 179, self.empty)
        cv2.createTrackbar("Sat Min","TrackBars", hsv_lower[1], 255, self.empty)
        cv2.createTrackbar("Sat Max","TrackBars", hsv_upper[1], 255, self.empty)
        cv2.createTrackbar("Val Min","TrackBars", hsv_lower[2], 255, self.empty)
        cv2.createTrackbar("Val Max","TrackBars", hsv_upper[2], 255, self.empty)
    ...
    def crop_color(self, image):
        ...
        while True:
            ...
            if self.cropping or self.get_rect:
                cv2.rectangle(clone_image, (self.x_start, self.y_start), (self.x_end, self.y_end), (0, 255, 0), 2)
        ...
        if (self.x_start, self.y_start) != (self.x_end, self.y_end):
            rect = clone_image[self.y_start:self.y_end, self.x_start:self.x_end]
    ...
    def calibrate_menu(self):
        ...
        while color_name != "cancel":
            ...
            if cnf_hsv_range is None:
                print("Color", color_name, "does not exists in 'cnf.txt' file.")
                resp = input("Type 'y' to contunue with other color, 'n' to exit: ")
                while resp not in ['n', 'y']:
                    resp = input("Wrong input (y/n): ")
                if resp == 'n':
                    break
            ...
            while folder_name not in all_directories:
                ...
                folder_name = input("Type folder name for color " + color_name + ": ")
            self.calibrate_color(folder_path=folder_name, color_name=color_name, new_photos=new_photos)
            ...
            for c in class_names:
                ...
                for or_file in os.listdir(or_class_dir):
                    if or_file.endswith(".png"):
                        ...
                        new_image = self.warp_image(image, top_crop=100, side_crop=10)
...
def main():
    ...
    # cvtool.wrap_all_images(new_dir="wrapped_imgs", img_dir="images_23_08_2022_11_35")
